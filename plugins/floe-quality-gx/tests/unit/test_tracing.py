"""Unit tests for OpenTelemetry tracing helpers in floe_quality_gx.tracing.

Tests cover tracer initialization, span creation, attribute setting, error
handling, and security (no PII/raw data in traces) for the GX quality plugin.
...
from floe_quality_gx.tracing import (
    ATTR_CHECKS_COUNT,
    ATTR_DATA_SOURCE,
    ATTR_FAIL_COUNT,
    ATTR_OPERATION,
...
class TestQualitySpan:
    ...
    def test_quality_span_sets_provider_attribute(
        ...
        Validates that the provider attribute is always set to 'great_expectations'.
        """
        provider, exporter = tracer_with_exporter
        tracer = provider.get_tracer(TRACER_NAME)
        ...
        assert attributes[ATTR_PROVIDER] == "great_expectations"
    ...
    def test_quality_span_sets_data_source_attribute(
        ...
        Validates that the data source (table name) attribute is recorded on
    ...
    def test_quality_span_error_sanitized(
        ...
        error_msg = "GX failed: password=secret123 at database"  # pragma: allowlist secret
    ...
    def test_get_tracer_returns_tracer(self) -> None:
        ...
        """
        with patch("floe_quality_gx.tracing._factory_get_tracer") as mock_factory:
...
class TestConstants:
    ...
    def test_tracer_name_constant(self) -> None:
        ...
        """
        assert TRACER_NAME == "floe.quality.gx"
