...
class TestTokenValidationValid:
    ...
    def test_valid_token_returns_valid_true(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            ...
            mock_jwt.get_unverified_header.return_value = {"kid": TEST_KEY_ID}
...
class TestTokenValidationInvalid:
    ...
    def test_invalid_signature_returns_valid_false(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            ...
            mock_jwt.decode.side_effect = InvalidSignatureError("Signature verification failed")
            mock_jwt.get_unverified_header.return_value = {"kid": TEST_KEY_ID}
            mock_jwt.exceptions = MagicMock()
            mock_jwt.exceptions.InvalidSignatureError = InvalidSignatureError
    ...
    def test_missing_required_claims_returns_valid_false(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            mock_jwt.decode.return_value = incomplete_claims
            mock_jwt.get_unverified_header.return_value = {"kid": TEST_KEY_ID}
    ...
    def test_wrong_issuer_returns_valid_false(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            ...
            mock_jwt.get_unverified_header.return_value = {"kid": TEST_KEY_ID}
            mock_jwt.exceptions = MagicMock()
            mock_jwt.exceptions.InvalidIssuerError = InvalidIssuerError
    ...
    def test_wrong_audience_returns_valid_false(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            ...
            mock_jwt.get_unverified_header.return_value = {"kid": TEST_KEY_ID}
            mock_jwt.exceptions = MagicMock()
            mock_jwt.exceptions.InvalidAudienceError = InvalidAudienceError
...
class TestTokenValidatorKeyLookup:
    ...
    def test_key_not_found_returns_valid_false(
        ...
        with patch("floe_identity_keycloak.token_validator.jwt") as mock_jwt:
            # Return a different key ID than what's in JWKS
            mock_jwt.get_unverified_header.return_value = {"kid": "unknown-key-id"}
