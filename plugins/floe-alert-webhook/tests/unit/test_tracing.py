"""Unit tests for OpenTelemetry tracing helpers in floe_alert_webhook.tracing.

Tests cover tracer initialization, span creation, attribute setting, error
handling, and security (no credentials in traces) for the webhook alert plugin.

Requirements Covered:
    - 6C-FR-021: Unit tests for alert tracing helpers
"""
...
from floe_alert_webhook.tracing import (
    ATTR_CHANNEL,
...
class TestAlertSpan:
    ...
    def test_alert_span_sets_channel_attribute(
        ...
        with alert_span(tracer, "send", channel="webhook"):
            pass

        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        attributes = dict(spans[0].attributes or {})
        assert attributes[ATTR_CHANNEL] == "webhook"
    ...
    def test_alert_span_no_credentials_in_attributes(
        ...
        Validates that webhook URLs do not contain embedded credentials.
        ...
        # Verify no attribute contains credentials (if URL had embedded creds)
        # pragma: allowlist secret
        for attr_value in attributes.values():
            assert "password123" not in str(attr_value), (
                f"Password found in span attribute: {attr_value}"
            )
            assert "user:password" not in str(attr_value), (
                f"Credentials found in span attribute: {attr_value}"
            )
    ...
    def test_alert_span_error_sanitized(
        ...
        # pragma: allowlist secret
        error_msg = "HTTP POST failed: api_key=secret-token-12345"
        ...
        assert "secret-token-12345" not in exception_message  # pragma: allowlist secret
    ...
    def test_alert_span_ok_status_on_success(
        ...
        with alert_span(tracer, "validate_config", channel="webhook"):
            pass
    ...
    def test_get_tracer_returns_tracer(self) -> None:
        ...
        """
        with patch("floe_alert_webhook.tracing._factory_get_tracer") as mock_factory:
...
class TestConstants:
    ...
    def test_tracer_name_constant(self) -> None:
        ...
        """
        assert TRACER_NAME == "floe.alert.webhook"
