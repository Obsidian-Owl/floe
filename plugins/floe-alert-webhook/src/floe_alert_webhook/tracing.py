"""OpenTelemetry tracing helpers for the webhook alert plugin.
...
Security:
    ...
    - Only sanitized destination info is recorded as span attributes
    - Error messages are sanitized before recording

Example:
    >>> from floe_alert_webhook.tracing import get_tracer, alert_span
    >>> tracer = get_tracer()
    >>> with alert_span(
    ...     tracer, "send", channel="webhook", destination="https://example.com"
    ... ) as span:
    ...     # perform operation
    ...     pass
...
TRACER_NAME = "floe.alert.webhook"
...
def get_tracer() -> trace.Tracer:
    ...
    the webhook alert plugin. If no tracer provider is configured or
...
def alert_span(
    ...
    *,
    ...
    Security: Only channel type and sanitized destination URLs are recorded,
    NEVER credentials embedded in URLs. Error messages are sanitized to
    redact any leaked credentials before recording on the span.

    Args:
        tracer: OpenTelemetry tracer instance.
        operation: Operation name (e.g., "send", "validate_config").
        channel: Alert channel type (e.g., "webhook").
        destination: Alert destination (webhook URL).
