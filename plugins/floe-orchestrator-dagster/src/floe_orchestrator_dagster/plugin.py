    ...
    def generate_entry_point_code(
        ...
        *,
        ...
        if lineage_enabled:
            ...
            if "from uuid import UUID, uuid4" not in stdlib_imports:
                stdlib_imports.append("from uuid import UUID, uuid4")
            thirdparty_imports.extend(
                [
                    "from floe_core.lineage.facets import TraceCorrelationFacetBuilder",
                    "from floe_orchestrator_dagster.lineage_extraction "
                    "import extract_dbt_model_lineage",
...
'''
        ...
        # Build the lineage body block (only when lineage_enabled)
        if lineage_enabled:
            asset_name = f"{safe_name}_dbt_assets"
            lineage_body = f'''    _logger = logging.getLogger(__name__)
    lineage = context.resources.lineage
    run_id = None
    ...
    try:
        run_id = lineage.emit_start("{asset_name}", run_facets=run_facets or None)
    except Exception:
...
{iceberg_post_build}
    # 4. Emit COMPLETE
    try:
        lineage.emit_complete(run_id, "{asset_name}")
    except Exception:
        _logger.warning("lineage_emit_complete_failed", exc_info=True)
'''
        else:
            lineage_body = f"""    yield from dbt.cli(["build"], context=context).stream()
{iceberg_post_build}"""
