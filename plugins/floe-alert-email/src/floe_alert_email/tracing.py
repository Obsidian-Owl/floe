"""OpenTelemetry tracing helpers for the Email alert plugin.

This module provides utilities for instrumenting email alert operations with
...
Example:
    >>> from floe_alert_email.tracing import get_tracer, alert_span
    >>> tracer = get_tracer()
    >>> with alert_span(
    ...     tracer,
    ...     "send_alert",
...
Requirements Covered:
    ...
    - 6C-FR-021: Unit tests for tracing helpers
...
def get_tracer() -> trace.Tracer:
    """Get the OpenTelemetry tracer for email alert operations.

    Returns a thread-safe tracer instance from the factory configured for
    the email alert plugin. If no tracer provider is configured or
...
def alert_span(
    ...
    *,
    ...
    Security: Only metadata is recorded, NEVER alert content or credentials.
    Error messages are sanitized to redact any leaked credentials before
    recording on the span.

    Args:
        tracer: OpenTelemetry tracer instance.
        operation: Operation name (e.g., "send_alert", "validate_config").
        channel: Alert channel name (e.g., "email").
        destination: Alert destination (e.g., recipient email address).
