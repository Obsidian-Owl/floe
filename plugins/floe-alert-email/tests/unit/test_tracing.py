"""Unit tests for OpenTelemetry tracing helpers in floe_alert_email.tracing.

Tests cover tracer initialization, span creation, attribute setting, error
handling, and security (no alert content in traces) for the Email alert plugin.

Requirements Covered:
    - 6C-FR-021: Unit tests for email alert tracing helpers
"""
...
class TestAlertSpan:
    ...
    def test_alert_span_sets_channel_attribute(
        ...
        with alert_span(tracer, "send_alert", channel="email"):
            pass

        spans = exporter.get_finished_spans()
        assert len(spans) == 1
        attributes = dict(spans[0].attributes or {})
        assert attributes[ATTR_CHANNEL] == "email"
    ...
    def test_alert_span_no_alert_content_in_attributes(
        ...
        Validates that only metadata (not alert content or credentials) is
        recorded. This is a critical security requirement (6C-FR-023).
        """
        provider, exporter = tracer_with_exporter
        tracer = provider.get_tracer(TRACER_NAME)

        fake_alert_content = "CRITICAL: SMTP password compromised"  # pragma: allowlist secret
    ...
    def test_alert_span_error_sanitized(
        ...
        error_msg = "SMTP auth failed: password=hunter2 for user"  # pragma: allowlist secret
    ...
    def test_alert_span_ok_status_on_success(
        ...
        with alert_span(tracer, "validate_config", channel="email"):
            pass
    ...
    def test_get_tracer_returns_tracer(self) -> None:
        ...
        """
        with patch("floe_alert_email.tracing._factory_get_tracer") as mock_factory:
...
class TestConstants:
    ...
    def test_tracer_name_constant(self) -> None:
        ...
        """
        assert TRACER_NAME == "floe.alert.email"
