...
| Customer 360 demo deployment | `make demo` in DevPod + Hetzner | Final Customer 360 validation record under `docs/validation/` | Not run on final merged release candidate |
| Customer 360 run trigger | `make demo-customer-360-run` in DevPod + Hetzner | Final Customer 360 validation record with `dagster.run_id` | Not run on final merged release candidate |
| Customer 360 validation | `make demo-customer-360-validate` in DevPod + Hetzner | Final Customer 360 validation record with business, storage, lineage, tracing, and Dagster evidence | Not run on final merged release candidate |
...
| DevPod + Hetzner E2E | `make devpod-test` | `test-artifacts/devpod-run-*/output.log` path linked from validation record | Not run on final merged release candidate |
