...
Excluded packages must not be registered as alpha PyPI pending publishers until
their composition path is proven.
...
| PyPI packages | Pending publisher registration for the 15 manifest packages | `pypi-publish.yml` publishes only `python_packages.publish` |
...
**Do not run until the release evidence bundle is complete and PyPI pending
publishers are registered for the 15 manifest packages.**
