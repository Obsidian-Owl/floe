...
catalog/storage dictionaries. Orchestrator plugins own Dagster asset creation
from compiled artifacts.

## Plugin Implementations

### dlt Plugin (Reference Embedded Option)
...
Ingestion destination wiring is provided by `CompiledArtifacts.deployment.ingestion`.
Ingestion plugins consume composed runtime bindings rather than accepting raw
catalog/storage dictionaries. Dagster-specific assets are generated by the
...
> Superseded implementation note (2026-04-25): The canonical Dagster runtime
> currently fails loudly when compiled JSON ingestion config contains workload
> sources. These Dagster asset examples remain the future design direction, but
> ingestion runtime execution is blocked until floe can construct executable dlt
> source objects from compiled artifacts.

```python
# Generated by the planned `floe compile` ingestion flow; not alpha-supported yet.
...
def github_events_asset(context: AssetExecutionContext, dlt: DagsterDltResource):
    runtime_binding = context.op_config["runtime_binding"]
    yield from dlt.run(
        source=github_reactions(access_token=context.op_config["github_token"]),
        destination=filesystem(**runtime_binding["destination_filesystem"]),
    )
```
