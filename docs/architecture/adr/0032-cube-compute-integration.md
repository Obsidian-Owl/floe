...
- **Reference compute path**: The documented DuckDB compute path should work with the semantic layer
...
3. **Compute-plugin-first**: The semantic layer uses the active ComputePlugin; the DuckDB reference path works out of the box for alpha examples
...
- **DuckDB reference path**: Works with the documented open-source alpha stack
...
│  │  │ (reference) │  │  (plugin)   │  │  (plugin)   │  │ (future)   │  │    │
│  │  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └─────┬──────┘  │    │
│  │         │                │                │               │         │    │
│  └─────────┼────────────────┼────────────────┼───────────────┼─────────┘    │
│            │                │                │               │              │
...
```javascript
// cube.js generated by the planned `floe compile` semantic-layer flow; not alpha-supported yet.
module.exports = {
