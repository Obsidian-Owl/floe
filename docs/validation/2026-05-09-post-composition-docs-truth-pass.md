...
| Authoritative current | `docs/reference/plugin-catalog.md` | Matches implementation truth that `PluginType` defines 14 categories and `PluginType.LINEAGE` is a code alias rather than an extra category. | Keep as the canonical public plugin category reference. |
