...
This module provides the Dagster Definitions entry point for the iot-telemetry
...
DUCKDB_PATH = "/tmp/iot_telemetry.duckdb"
...
def _export_dbt_to_iceberg(context: Any) -> None:
    ...
    product_namespace = "iot_telemetry"
...
def iot_telemetry_dbt_assets(context, dbt: DbtCliResource):
    """Execute iot-telemetry dbt models as Dagster assets.
    ...
    try:
        run_id = lineage.emit_start("iot_telemetry_dbt_assets", run_facets=run_facets or None)
    except Exception:
    ...
    # 4. Emit COMPLETE
    try:
        lineage.emit_complete(run_id, "iot_telemetry_dbt_assets")
    except Exception:
...
defs = Definitions(
    assets=[iot_telemetry_dbt_assets],
    resources={
        "dbt": DbtCliResource(
