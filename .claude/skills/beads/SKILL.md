...
**When Linear integration is enabled** (via `.beads/config.yaml`), Beads acts as a local cache of Linear issues with bidirectional sync.

### Architecture Pattern

**Linear-First**:
- Linear is the source of truth for issue status, assignments, priorities
- Beads is a local cache synced via `bd linear sync --pull`
- All status updates should happen in Linear first, then sync to Beads

### Two Sync Operations

**Understand the difference between sync types:**
...
**When to use which:**
- Starting session with Linear: `bd linear sync --pull`
- Ending session (ephemeral branch): `bd sync` then `bd linear sync`
- Just working locally: No sync needed (auto-commits to git)

### Comment Syncing Limitation ⚠️
...
```bash
bd close issue-123 --reason "Implemented JWT validation with tests passing"
# ✅ Reason saved in Beads
# ❌ Reason NOT synced to Linear
```

**Workaround:** Manually create Linear comment via MCP after closing:
```bash
# 1. Close in Linear (via SpecKit or MCP)
...
**Why this matters:** Without manual comment creation, closure context is lost in Linear when other team members view the issue.

### Session Close Protocol (Linear + Ephemeral Branches)

**When working on ephemeral branches with Linear integration:**

```bash
# End of session checklist:
1. git status                    # Check what changed
2. git add <files>               # Stage code changes
3. bd sync                       # Commit beads changes to git
...
**Why both syncs:** `bd sync` preserves Beads state in git for branch collaboration, `bd linear sync` ensures you have latest Linear updates.

### SpecKit Integration

**When using SpecKit commands** (`/speckit.implement`, `/speckit.taskstolinear`), follow these patterns:

- **Task Creation**: `/speckit.taskstolinear` creates Linear issues and syncs to Beads
- **Task Implementation**: `/speckit.implement` updates Linear first, then Beads
- **Closure Comments**: SpecKit commands include manual MCP comment creation (see `/speckit.implement` Step 6)

**For complete Linear workflow details, read:** `docs/guides/linear-workflow.md`
