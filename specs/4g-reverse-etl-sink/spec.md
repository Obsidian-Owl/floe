...
- **FR-009**: `DltIngestionPlugin.get_source_config(catalog_config)` MUST return configuration for reading from the Iceberg Gold layer via the Polaris catalog, including catalog URI, warehouse, credentials, namespace, and table selection required by reverse ETL jobs.
