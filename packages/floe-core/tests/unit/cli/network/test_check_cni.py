...
class TestCheckCniCommandOptions:
    ...
    def test_accepts_kubeconfig_option(
        ...
        with patch(
            "floe_core.cli.network.check_cni._load_kubernetes_client"
        ) as mock_load:
...
class TestCniSupportMatrix:
    ...
    def test_azure_cni_supported(self) -> None:
        ...
        assert (
            "Support with Network Policy Manager" in CNI_SUPPORT["azure-cni"]["notes"]
        )
...
class TestLoadKubernetesClient:
    ...
    def test_uses_context_parameter(
        self, mock_k8s_config: MagicMock, tmp_path: Path
    ) -> None:
...
class TestDetectCni:
    ...
    def test_detects_calico_from_daemonset(
        ...
        mock_apps_api.list_namespaced_daemon_set.return_value = (
