...
class CompositionResolver:
    ...
    def _validate_storage_for_catalog(
        ...
        """Validate storage capabilities against peer plugin requirements."""
        issues: list[CompositionIssue] = []
        storage_protocols = list(storage.capabilities.protocols)
        required_protocols = list(requirement.requirements.protocols)
        requester = f"{requirement.plugin_type} {requirement.plugin_name}"
        ...
        required_modes = list(requirement.requirements.credential_modes)
