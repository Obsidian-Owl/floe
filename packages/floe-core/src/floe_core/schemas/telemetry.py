"""Telemetry configuration schemas (Pydantic v2).
...
MOVED from floe_core/telemetry/config.py to break circular dependency:
  schemas -> telemetry -> plugins -> schemas

TelemetryConfig is a Pydantic schema (data definition), so it belongs in
the schemas module. The telemetry/config.py module re-exports these for
...
# Public API - all classes moved from telemetry/config.py
__all__ = [
    "ResourceAttributes",
    "TelemetryAuth",
